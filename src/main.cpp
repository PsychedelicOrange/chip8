...
inline void loop_frequency(double frequency_hz)
{
    ...
    double work_time_ms = (sleep_begin - work_begin) * 1000;
    if(work_time_ms < 1000.0/frequency_hz)
    {
        std::this_thread::sleep_for(std::chrono::duration<double,std::milli>(1000.0/frequency_hz - work_time_ms));
    }
...
int main(int argc, char** argv)
{
    ...
    {
        loop_frequency(chip8::TIMER_FREQUENCY);
